...
_ROUGE_PATH = '/home/thanhthuong/miniconda3/envs/ml/lib/python3.7/site-packages/rouge_metric/RELEASE-1.5.5'
...
def get_candidates(tokenizer, cls, sep_id, idx):
    ...
    # sent_id = sent_ids[idx]['sent_id'][:1]
    # indices = [(i,) for i in sent_id]  # Chọn từng câu một

    # if len(sent_id) < 1:
    #      indices = [(sent_id[0],)]  # Lấy câu đầu tiên nếu số lượng câu < 1
    ...
    #     i.sort()
    #     # write dec
    #     dec = [data['text'][j] for j in i]  # Lấy một câu duy nhất
    ...
    for summary in candidate_summary:
        token_ids = tokenizer.encode(summary, add_special_tokens=True)[:(MAX_LEN - 1)]
        token_ids += sep_id
    ...
    text = text[:MAX_LEN]
    text = ' '.join(text)
    token_ids = tokenizer.encode(text, add_special_tokens=True)[:(MAX_LEN - 1)]
    token_ids += sep_id
...
def get_candidates_mp(args):
    ...
    cls, sep = '<s>', '</s>'
    sep_id = tokenizer.encode(sep, add_special_tokens=True)
